...
def run_experiment(cfgs: DictConfig) -> None:
    """
    ...
    Returns:
        None : Future might return a float for hyperparameter tuning.
    """
    ...
    if cfgs.exp.is_cuda:
        ...
        if device == "cpu":
            raise RuntimeError("CUDA unavailable, require CUDA for experiment")
    ...
    if run_obj is None:
        raise ValueError(f"{cfgs.exp.algorithm.capitalize()} module not found")
