...
class TestMain:
    ...
    def test_main_hydra_decorator(self):
        ...
        assert main.main.__wrapped__.__name__ == "main", (
            "main function \
                is not named 'main'"
        )
        assert hasattr(main.main, "__wrapped__"), (
            "main function \
                is not decorated with hydra.main"
        )
        assert callable(main.main.__wrapped__), (
            "main function \
                is not callable"
        )
        assert main.main.__annotations__["cfg"] == "DictConfig", (
            "main function \
